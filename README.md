...
`serve --once` runs one scheduler decision for deterministic readiness checks. Long-running production uses plain `serve`. Canary or E2E scheduled configs may use short operator intervals down to `5m` and set `schedule.max_live_publishes_per_tick: 1` so a scheduler tick fails before posting if more than one live publish is eligible.
