...
`argus status` exposes active scheduled-delivery outages, immutable exact-cause events, both notification receipts, credential-recovery evidence, and the scheduled Subspace message-id gate. An outage clears only after post-outage durable credential recovery (persisted reauth or an authenticated durable-identity join) and a real scheduled live publish with a non-empty Subspace message id. On restart, Argus reconciles a persisted successful reauth or committed scheduled publish into the active outage before applying the same clear predicate. Manual/canary success, packaging, source fetch, credential recovery alone, and success without a message id do not clear it.
