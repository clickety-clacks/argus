...
The identity JSON contains the existing identity name, base64url Ed25519 public key, and base64url raw private key. The session JSON is created and atomically replaced by Argus with mode `0600`; it is target-scoped and contains the matching agent id, nullable token/expiry, issuance time, monotonic reauth generation, and last reauth result. Argus refuses a mismatched identity/session pair. Missing tokens, approaching finite expiry, `TOKEN_INVALID`, and `TOKEN_REVOKED` use the Subspace challenge-response reauth endpoints automatically. Do not generate a replacement identity for an existing Racter deployment.
...
`argus status` exposes active scheduled-delivery outages, immutable exact-cause events, both notification receipts, credential-recovery evidence, and the scheduled Subspace message-id gate. An auth-caused outage requires a later durable reauth generation; a non-auth outage requires a later authenticated durable-identity join. Both also require a real scheduled live publish with a non-empty Subspace message id. On restart, Argus uses the outage's captured reauth generation and publish-attempt row boundary to reconcile only evidence created after the outage opened. Manual/canary success, packaging, source fetch, credential recovery alone, success without a message id, and earlier same-second publish success do not clear it.

Legacy scheduled-cycle and Subspace circuit alerts cannot announce recovery while that target-scoped outage remains active. Outage-linked legacy alert rows resolve in the same SQLite transaction as the canonical clear and retain the scheduled Subspace message id in their recovery evidence. Argus does not emit the optional legacy recovery page for these rows; the durable first-outage Pushover and operator-alert receipts remain authoritative.
